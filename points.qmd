...
We should also be intentional about checking if the geocoder gave us any `NA` values:

```{r}
any(is.na(geoCodedClinics[c('longitude', 'latitude')]))
```

All of our data geocoded correctly, so we should be set to move forwards. If we *did* have observations that did not geocode correctly, then we would need to investigate them further, possibly by searching the address and pulling the lat/long using Google Maps, and then inputting it manually.
...
tm_shape(Chi_Zipsf) + 
  ...
  tm_add_legend("symbol", col = "gray20", size = 0.2, labels = "Methadone MOUD") +
...
## Zip Code Choropleth Map
  ...
  tm_shape(AffHousingSf) + tm_bubbles("Units",
                                      size.legend = tm_legend(title='Affordable\nRental Units')) + 
